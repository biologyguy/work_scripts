...
def ConvertSubMatrix(sub_matrix):
        ...
        for next_aa in converted_sub_matrix:
            if next_aa == "-":
                converted_sub_matrix[next_aa] = {"A": (-4), "C": (-4), "D": (-4), "E": (-4), "F": (-4), "G": (-4), "H": (-4), "I": (-4), "K": (-4), "L": (-4), "M": (-4), "N": (-4), "P": (-4), "Q": (-4), "R": (-4), "S": (-4), "T": (-4), "V": (-4), "W": (-4), "Y": (-4), "X": (-4), "-": (-4)}
            elif next_aa == "X":
                converted_sub_matrix[next_aa] = {"A": (-1), "C": (-1), "D": (-1), "E": (-1), "F": (-1), "G": (-1), "H": (-1), "I": (-1), "K": (-1), "L": (-1), "M": (-1), "N": (-1), "P": (-1), "Q": (-1), "R": (-1), "S": (-1), "T": (-1), "V": (-1), "W": (-1), "Y": (-1), "X": (-1), "-": (-4)}            
            else:
                converted_sub_matrix[next_aa] = {"A": 0, "C": 0, "D": 0, "E": 0, "F": 0, "G": 0, "H": 0, "I": 0, "K": 0, "L": 0, "M": 0, "N": 0, "P": 0, "Q": 0, "R": 0, "S": 0, "T": 0, "V": 0, "W": 0, "Y": 0, "X": (-1), "-": (-4)}
        ...
        for i in sub_matrix:
            ...
            converted_sub_matrix[x][y] = sub_matrix[i]
            converted_sub_matrix[y][x] = sub_matrix[i]
               
        return converted_sub_matrix
 
 
def comp(aln1, aln2, matrix):
    seq1 = list(aln1.seq)
    seq2 = list(aln2.seq)
    ...
    for index in range(len(seq1)):
        if seq1[index] == '-' and seq2[index] == '-':
            ...
            output['len'] += 1
            continue
        elif int(matrix[seq1[index]][seq2[index]]) >= 0:
            output['sim'] += 1.0
            output['len'] += 1
            continue
        else:
...
matrix = ConvertSubMatrix(MatrixInfo.blosum62)
output = {}  # [{(Name1, Name2): {'id':%ID, 'sim':%SIM, 'len':LEN}]
...
if in_args.internal:  
    for i in align_obj:
        if re.search(in_args.search_string,i.id) != None:
            group1.append(i)
            group2.append(i)
            clip_group1 = True
...
for i in group1:
    if clip_group1:
        del group2[0]
    for j in group2:
        #print "\rProcessing " + str(counter) + " of " + str(number_comparisons),
        if i.id == j.id:
            continue
        comparison = comp(i, j, matrix)
        output[(i.id, j.id)] = comparison
        
        print(i.id, "\t", j.id, "\t", round(comparison['id'], 3), "\t", round(comparison['sim'], 3), "\t", comparison['len'])
                

#print output[('Homo_sapiens_Connexin_ENSP00000362464', 'Danio_rerio_Connexin_ENSDARP00000108204')]